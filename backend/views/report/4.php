...
$orders_all = $db->createCommand('SELECT count(*) AS orders, sum(qty) as qty, product_group.name as product_group_name, customer.region FROM so 
...
$offers_all = $db->createCommand('SELECT count(*) AS orders, sum(qty) as qty, product_group.name as product_group_name, customer.region FROM so 
    left join product_group on so.product_group_id = product_group.id
    left join customer on so.customer_id = customer.id
